...
void loop() {
  ...
  Serial.print(F("VC4:")); Serial.print(vc4);Serial.print(F("mV\n"));

  Serial.print(F("Stack:")); Serial.print(stackV);Serial.print(F("mV\n"));
  if (fStat & 0x04) {
    Serial.print(F("Pack:")); Serial.print(packV);Serial.print(F("0mV"));
  } else {
  ...
  if (!(fStat & 0x04)) {
    ...
    // Every 10th cycle, re-check and force MFG status
    static uint8_t cycleCount = 0;
    cycleCount++;
    if (cycleCount >= 10) {
      cycleCount = 0;
      uint16_t mfg = readMfgStatus();
      if (!(mfg & 0x0002)) {
        Serial.println(F("[ACTION] Re-forcing MFG FET_EN..."));
        setRuntimeMfgStatus(0x0006);
      }
    }
    ...
    Serial.print(F("[ACTION] FET after cmd: 0x"));
    Serial.println(fCheck, HEX);
  }

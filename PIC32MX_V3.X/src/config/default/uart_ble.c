/*******************************************************************************
  ...
  Summary:
    UART1 command reception from a BLE module on PIC32MX274F256B.

  Description:
    Receives plain-text commands ("start", "stop") from a BLE module over
    UART1 at 115200 baud. Uses a circular queue to decouple the hardware RX
    ...
    logic and sends the acknowledgement response back over UART1.
    ...
    MODULE DEPENDENCIES:
      ...
      definitions.h- UART1_* PLIB functions from MCC Harmony
    -------------------------------------------------------------------------
*******************************************************************************/
...
#include "definitions.h"    // UART1_* PLIB functions
 ...
 * bleByte
 *
 * Single-byte landing buffer passed to UART1_Read(). The UART1 PLIB writes
 * the received byte here and fires UART1_RX_Callback(). Do not read this
 ...
 *   bleQueueHead - written by UART1_RX_Callback() (interrupt context)
...
{
    // Register our callback with the UART1 PLIB.
    // UART1_RX_Callback will be called each time one byte is received.
    UART1_ReadCallbackRegister(UART1_RX_Callback, 0);

    // Wait for any in-progress read to finish (should not be busy at startup,
    // but guard here for safety)
    while (UART1_ReadIsBusy());
...
void UART_BLE_Process(void)
{
    ...
    while (bleQueueTail != bleQueueHead)
    {
        ...
        // NOTE: characters are NOT echoed on this channel.
        // BLE central devices send commands programmatically and do not
        // need to see their own characters reflected back.
        ...
        if (c == '\r' || c == '\n')
        {
            ...
            if (bleRxIndex > 0U)
            {
                ...
                // back over UART1 via UART_BLE_Send().
...
void UART_BLE_Send(const char *str)
{
    ...
    // Initiate the transmission. UART1_Write() starts the TX interrupt chain;
    // the PLIB handles the rest in the background.
    UART1_Write(bleTxBuffer, strlen(bleTxBuffer));
}

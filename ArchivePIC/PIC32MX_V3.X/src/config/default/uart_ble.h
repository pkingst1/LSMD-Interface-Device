/*******************************************************************************
  ...
  Summary:
    Public interface for BLE module command reception on UART1.

  Description:
    Provides the initialisation, RX callback, main-loop processing function,
    and transmit helper for the BLE module connected on UART1 at 115200 baud.
    ...
    HARDWARE:
      Peripheral : UART1
      Baud rate  : 115200
      Device     : BLE module (exact module configurable via UART1 MCC settings)
    -------------------------------------------------------------------------

    -------------------------------------------------------------------------
    TO CHANGE THE BAUD RATE:
      Reconfigure UART1 in MCC Harmony and regenerate plib_uart1.c/.h.
 ...
 * UART_BLE_Init
 *
 * Registers the UART1 RX interrupt callback and arms the first 1-byte read.
 * Must be called once during startup, after SYS_Initialize(), before the
 * main loop begins.
 *
 * Internally calls:
 *   UART1_ReadCallbackRegister() - registers UART1_RX_Callback
 *   UART1_Read()                 - arms the first byte reception
 */
 ...
 * UART1_RX_Callback
 *
 * Hardware interrupt callback fired by the UART1 PLIB each time a byte is
 ...
 *   2. Re-arms UART1_Read() for the next incoming byte
 *
 ...
 *      Command_Dispatch(buffer, CMD_SOURCE_UART1_BLE)
 ...
 * UART_BLE_Send
 *
 * Sends a null-terminated string over UART1 to the BLE module.
 * Blocks until any in-progress UART1 transmission completes, then
